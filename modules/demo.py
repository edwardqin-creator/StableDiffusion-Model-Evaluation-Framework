...
def main(model_pipe=None):
    ...
    if model_pipe is None:
        model_pipe = DiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
        model_pipe.to("cuda")
...
def performance_record(pipe, prompt, height, width, steps):
    ...
    log_dir = "./log/StableDiffusionV1-5_Saved" #这里请修改为你自己的存放目录，用于存储生成的json文件
